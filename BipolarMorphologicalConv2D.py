...
class BipolarMorphologicalConv2D(Layer):
    ...
    def call(self, x):
        ...
        k1_for_patches = tf.reshape(
            self.k1, [filter_height * filter_width * in_channels, out_channels]
        )
        k2_for_patches = tf.reshape(
            self.k2, [filter_height * filter_width * in_channels, out_channels]
        )

        x1_patches = tf.extract_image_patches(
            x,
        ...
        x1_patches = tf.maximum(
            tf.log(tf.maximum(x1_patches, self.input_shift)), tf.float32.min
        )
        ...
        x2_patches = tf.maximum(
            tf.log(tf.maximum(x2_patches, self.input_shift)), tf.float32.min
        )
    ...
    def compute_output_shape(self, input_shape):
        if self.padding[0] == "VALID":
            return (
                input_shape[0],
                (input_shape[1] - self.kernel_shape[1] + 1) // self.strides[0],
                (input_shape[2] - self.kernel_shape[0] + 1) // self.strides[1],
                self.kernel_shape[3],
            )
        else:
            return (
